...
export default function ProjectTypesPage() {
  ...
  const handleDelete = async (id: string) => {
    if (!confirm('この工事種別を削除しますか？')) return
        ...
        <div className="max-w-7xl mx-auto px-3 sm:px-6 py-3">
          <div className="flex justify-between items-center">
            <Link href="/admin" className="flex items-center space-x-2 hover:opacity-80 transition-opacity">
              <div className="w-9 h-9 sm:w-10 sm:h-10 rounded-lg bg-orange-600 flex items-center justify-center">
                <Layers className="w-5 h-5 sm:w-6 sm:h-6 text-white" />
              </div>
              <div>
                <h1 className="text-lg sm:text-xl font-bold text-gray-900">工事種別管理</h1>
                <p className="text-xs text-gray-500 hidden sm:block">工事種別マスタの登録・管理</p>
              </div>
            </Link>
          ...
          <div className="px-4 sm:px-6 py-4 border-b border-slate-200 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
            ...
            <div className="flex items-center space-x-2">
              <button
                ...
                className="inline-flex items-center px-3 py-2 text-sm font-medium bg-orange-600 text-white rounded-lg hover:bg-orange-700 transition-colors"
              >
              ...
              <div className="flex items-end gap-3">
                <div className="flex-1">
                  <label className="block text-sm font-medium text-gray-700 mb-1">
                    工事種別名 <span className="text-red-500">*</span>
                  </label>
                  <input
                    type="text"
                ...
                <button
                  ...
                  disabled={saving}
                  className="inline-flex items-center px-4 py-2 text-sm font-medium bg-orange-600 text-white rounded-lg hover:bg-orange-700 disabled:bg-gray-400"
                >
          ...
          <div className="overflow-x-auto">
            <table className="w-full">
              <thead className="bg-gray-50 border-b border-slate-200">
                <tr>
                  <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase">工事種別名</th>
                ...
                {projectTypes.length === 0 ? (
                  <tr>
                    <td colSpan={3} className="px-6 py-12 text-center">
                      ...
                      <p className="text-gray-500 mb-2">工事種別が登録されていません</p>
                  ...
                  projectTypes.map(projectType => (
                    <tr
                      ...
                      className={`hover:bg-gray-50 transition-colors ${
                        !projectType.isActive ? 'opacity-50' : ''
                      }`}
                    >
                      <td className="px-6 py-4">
                        <span className="text-sm font-medium text-gray-900">{projectType.name}</span>
                      </td>
                      <td className="px-6 py-4">
                        <span
